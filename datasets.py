...
def create_train_loader():
    ds = ImageFolder(os.path.join(config.DATASET_PATH, "Train"), transform=transform)
    return DataLoader(ds, batch_size=config.TRAINING_BATCH_SIZE, shuffle=True, drop_last=True)

def create_test_loader():
    ds = ImageFolder(os.path.join(config.DATASET_PATH, "Test"), transform=transform)
    return DataLoader(ds, batch_size=config.TEST_BATCH_SIZE, shuffle=True, drop_last=True)

def create_validation_loader():
    ds = ImageFolder(os.path.join(config.DATASET_PATH, "Validation"), transform=transform)
